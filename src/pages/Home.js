...
export default function Home() {
  ...
  useEffect(() => {
    ...
    const unsub = onSnapshot(q, snap => {
      ...
      setPosts([...firestorePosts, ...SEED_POSTS]);
      setLoading(false);
    });
    return () => unsub();
  }, []);
    ...
    <>
      ...
      <div className="app-layout">
        ...
        <aside className="right-sidebar">
          <ProfileCompletion />
          <Notifications
            posts={posts.filter(p => p.isReal)}
            currentUserId={user?.uid}
          />
          <div className="trending-panel">
            <div className="panel-title">🔥 Trending Topics</div>
            {TRENDING.map(t => (
              <div key={t.tag} className="hashtag-item">
                <div className="hashtag-name">{t.tag}</div>
                <div className="hashtag-count">{t.count}</div>
              </div>
            ))}
          </div>
        </aside>
